...
struct sigaction actionDri;
struct itimerval timer;
task_t *scheduler();
void dispatcher_body(void *arg);
void tratador(int signum);
void tratSinal(int signum);
...
void pingpong_init ()
{
    ...
    actionDri.sa_handler = tratSinal;
    sigemptyset(&action.sa_mask);
    action.sa_flags = 0;
    if(sigaction(SIGUSR1, &actionDri, 0) < 0){
        perror("Erro em sigaction: ");
        exit(1);
    }
...
void tratSinal(int sigunm){
	disco.signal++;
	diskDriver.status = READY;
	queue_append((queue_t **)&taskProntas, (queue_t *)&diskDriver);
	diskDriver.fila = (queue_t**)&taskProntas;
}
...
void diskDriverBody(void* args){
	while(1){
		sem_down(&disco.semDisco);

		if(disco.signal>0){
			
			task_t* pedinte = disco.filaPedidos->dono;
			queue_remove((queue_t**)&disco.filaPedidos, (queue_t*)disco.filaPedidos);
			queue_append((queue_t**)&taskProntas, (queue_t*) pedinte);
			pedinte->fila = (queue_t**) &taskProntas;
			pedinte->status = READY;
			disco.signal--;
		}
		if(disco.filaPedidos){
			filaPedidos_t* pedido = disco.filaPedidos;
			disk_cmd(pedido->cmd, pedido->bloco, pedido->buffer);
			disco.numPedidos--;
		}
		sem_up(&disco.semDisco);
		taskAtual->status = SUSP;

		if(taskMain.status == ENDED){
			task_exit(0);
		}
		task_switch(&dispatcher);
	}
...
int diskdriver_init (int *numBlocks, int *blockSize){
	if(numBlocks && blockSize){
		...
		*numBlocks = disk_cmd(DISK_CMD_DISKSIZE, 0, 0);
		sem_create(&disco.semDisco, 1);
		disco.signal = 0;
		disco.numPedidos = 0;
		task_create(&diskDriver, diskDriverBody, "Gerenciador do Disco ");
		taskUser--;
		return 0;
	}
	return -1;
...
int disk_block_read (int block, void *buffer) {
	...
	pedido.bloco = block;
	pedido.atend = 0;
	pedido.cmd = DISK_CMD_READ;
	pedido.buffer = buffer;
	pedido.dono = taskAtual;

	queue_append((queue_t**)&disco.filaPedidos, (queue_t*)&pedido);
		disco.numPedidos++;

	if(diskDriver.status == SUSP){
		diskDriver.status = READY;
		queue_append((queue_t **) &taskProntas, (queue_t *) &diskDriver);
		diskDriver.fila = (queue_t**)&taskProntas;
	}
