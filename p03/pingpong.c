...
task_t *scheduler(){ //função scheduler que implementa politica FIFO
	task_t *taskEscolhida = NULL;
	if(((queue_t *) taskProntas) > 0){ //se houver alguma tarefa pronta para ser executada
		taskEscolhida = taskProntas;
		return taskEscolhida;
    }
    else{
    	return 0;
  	}
